...
for n=1:length(Gamma1)
    ...
    a=gamma1*MMSE_64_QAM_23_new(4*gamma1/3);  %Original lower bound for external bisection
    b=gamma2*MMSE_64_QAM_23_new(4*gamma1/3);  %Original upper bound for external bisection
...
for n=1:length(Gamma1)
    ...
    d_8_1=[0:7;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(p1(n)/21);  % Distance matrix
    z1=sigma1*z;
    
    f1=zeros(length(d_8_1),1);
    b1=zeros(1,length(d_8_1));
    x1=zeros(N,1);
    
    for m=1:N  % Number of pages
        for j=1:length(d_8_1)  % Number of columns
            for i=1:length(d_8_1)  % Number of rows
                f1(i)=exp(-d_8_1(i,j)^2/(2*sigma1^2)-z1(m)*d_8_1(i,j)/sigma1^2);
            end
            b1(j)=log(sum(f1))/log(2);  % First sum over i
        end
        x1(m)=sum(b1);  % Second sum over j
    end
    C_2_1(n)=6-0.25*mean(x1);  % Capacity of 64-QAM
    %------------------------Constellation wf------------------------------
    d_8_12=[0:7;
        -1:6;
        -2:5;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(p_1(n)/21);  % Distance matrix
    
    f12=zeros(length(d_8_12),1);
    b12=zeros(1,length(d_8_12));
    x12=zeros(N,1);
    
    for m=1:N  % Number of pages
        for j=1:length(d_8_12)  % Number of columns
            for i=1:length(d_8_12)  % Number of rows
                f12(i)=exp(-d_8_12(i,j)^2/(2*sigma1^2)-z1(m)*d_8_12(i,j)/sigma1^2);
            end
            b12(j)=log(sum(f12))/log(2);  % First sum over i
        end
        x12(m)=sum(b12);  % Second sum over j
    end
    C_2_12(n)=6-0.25*mean(x12);  % Capacity of 64-QAM
    ...
    d_8_14=[0:7;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(1/21);  % Distance matrix
    
    f14=zeros(length(d_8_14),1);
    b14=zeros(1,length(d_8_14));
    x14=zeros(N,1);
    
    for m=1:N  % Number of pages
        for j=1:length(d_8_14)  % Number of columns
            for i=1:length(d_8_14)  % Number of rows
                f14(i)=exp(-d_8_14(i,j)^2/(2*sigma1^2)-z1(m)*d_8_14(i,j)/sigma1^2);
            end
            b14(j)=log(sum(f14))/log(2);  % First sum over i
        end
        x14(m)=sum(b14);  % Second sum over j
    end
    C_2_14(n)=6-0.25*mean(x14);  % Capacity of 64-QAM
    %---------------------------Regular WF---------------------------------
    d_8_15=[0:7;
        -1:6;
        -2:5;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(p11(n)/21);  % Distance matrix
    
    f15=zeros(length(d_8_15),1);
    b15=zeros(1,length(d_8_15));
    x15=zeros(N,1);
    
    for m=1:N  % Number of pages
        for j=1:length(d_8_15)  % Number of columns
            for i=1:length(d_8_15)  % Number of rows
                f15(i)=exp(-d_8_15(i,j)^2/(2*sigma1^2)-z1(m)*d_8_15(i,j)/sigma1^2);
            end
            b15(j)=log(sum(f15))/log(2);  % First sum over i
        end
        x15(m)=sum(b15);  % Second sum over j
    end
    C_2_15(n)=6-0.25*mean(x15);  % Capacity of 64-QAM
end
%-----------------------------------p2-------------------------------------
...
for n=1:length(Gamma1)
    ...
    d_8_1=[0:7;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(p2(n)/21);  % Distance matrix
    ...
    for m=1:N  % Number of pages
        for j=1:length(d_8_1)  % Number of columns
            for i=1:length(d_8_1)  % Number of rows
                f1(i)=exp(-d_8_1(i,j)^2/(2*sigma2^2)-z2(m)*d_8_1(i,j)/sigma2^2);
            end
            b1(j)=log(sum(f1))/log(2);  % First sum over i
        end
        x1(m)=sum(b1);  % Second sum over j
    end
    C_2_2(n)=6-0.25*mean(x1);  % Capacity of 64-QAM
    %------------------------Constellation wf------------------------------
    d_8_12=[0:7;
        -1:6;
        -2:5;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(p_2(n)/21);  % Distance matrix
    ...
    for m=1:N  % Number of pages
        for j=1:length(d_8_12)  % Number of columns
            for i=1:length(d_8_12)  % Number of rows
                f12(i)=exp(-d_8_12(i,j)^2/(2*sigma2^2)-z2(m)*d_8_12(i,j)/sigma2^2);
            end
            b12(j)=log(sum(f12))/log(2);  % First sum over i
        end
        x12(m)=sum(b12);  % Second sum over j
    end
    C_2_22(n)=6-0.25*mean(x12);  % Capacity of 64-QAM
    %------------------------Stronger channel------------------------------
    d_8_13=[0:7;
        -1:6;
        -2:5;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(2/21);  % Distance matrix
    
    f13=zeros(length(d_8_13),1);
    b13=zeros(1,length(d_8_13));
    x13=zeros(N,1);
    
    for m=1:N  % Number of pages
        for j=1:length(d_8_13)  % Number of columns
            for i=1:length(d_8_13)  % Number of rows
                f13(i)=exp(-d_8_13(i,j)^2/(2*sigma2^2)-z2(m)*d_8_13(i,j)/sigma2^2);
            end
            b13(j)=log(sum(f13))/log(2);  % First sum over i
        end
        x13(m)=sum(b13);  % Second sum over j
    end
    C_2_13(n)=6-0.25*mean(x13);  % Capacity of 64-QAM
    %--------------------Uniform power allocation--------------------------
    d_8_14=[0:7;
        -1:6;
        -2:5;
    ...
    for m=1:N  % Number of pages
        for j=1:length(d_8_14)  % Number of columns
            for i=1:length(d_8_14)  % Number of rows
                f14(i)=exp(-d_8_14(i,j)^2/(2*sigma2^2)-z2(m)*d_8_14(i,j)/sigma2^2);
            end
            b14(j)=log(sum(f14))/log(2);  % First sum over i
        end
        x14(m)=sum(b14);  % Second sum over j
    end
    C_2_24(n)=6-0.25*mean(x14);  % Capacity of 64-QAM
    %---------------------------Regular WF---------------------------------
    d_8_15=[0:7;
        -1:6;
        -2:5;
        ...
        -5:2;
        -6:1;
        -7:0]'*2*sqrt(p21(n)/21);  % Distance matrix
    ...
    for m=1:N  % Number of pages
        for j=1:length(d_8_15)  % Number of columns
            for i=1:length(d_8_15)  % Number of rows
                f15(i)=exp(-d_8_15(i,j)^2/(2*sigma2^2)-z2(m)*d_8_15(i,j)/sigma2^2);
            end
            b15(j)=log(sum(f15))/log(2);  % First sum over i
        end
        x15(m)=sum(b15);  % Second sum over j
    end
    C_2_25(n)=6-0.25*mean(x15);  % Capacity of 64-QAM
end
