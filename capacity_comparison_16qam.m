...
Gamma1_dB=-20:25;
Gamma2_dB=Gamma1_dB+20;   %xx-fold gain
...
for n=1:length(p)
    ...
    for k=1:max2+1
        ...
        %p_1c=regular_wf1(2*p(n),g1,c);   %Power allocation at c
        %p_2c=regular_wf1(2*p(n),g2,c);
        
        %p_1d=regular_wf1(2*p(n),g1,d);   %Power allocation at d
        %p_2d=regular_wf1(2*p(n),g2,d);
        
        p_1l_new=regular_wf1(2*p(n),g1,lamda_new);   %Power allocation at lamda
        p_2l_new=regular_wf1(2*p(n),g2,lamda_new);
...
for n=1:length(Gamma1)
    ...
    d_4_15=[0:3;
        -1:2;
        -2:1;
        -3:0]'*2*sqrt(p11(n)/5);  % Distance matrix
    
    f15=zeros(length(d_4_15),1);
    b15=zeros(1,length(d_4_15));
    x15=zeros(N,1);
    
    for m=1:N  % Number of pages
        for j=1:length(d_4_15)  % Number of columns
            for i=1:length(d_4_15)  % Number of rows
                f15(i)=exp(-d_4_15(i,j)^2/(2*sigma1^2)-z1(m)*d_4_15(i,j)/sigma1^2);
            end
            b15(j)=log(sum(f15))/log(2);  % First sum over i
        end
        x15(m)=sum(b15);  % Second sum over j
    end
    C_2_15(n)=4-0.5*mean(x15);  % Capacity of 16-QAM
end
...
for n=1:length(Gamma1)
    ...
    d_4_13=[0:3;
        -1:2;
        -2:1;
        -3:0]'*2*sqrt(2/5);  % Distance matrix
    ...
    for m=1:N  % Number of pages
        for j=1:length(d_4_13)  % Number of columns
            for i=1:length(d_4_13)  % Number of rows
                f13(i)=exp(-d_4_13(i,j)^2/(2*sigma2^2)-z2(m)*d_4_13(i,j)/sigma2^2);
            end
    ...
    d_4_15=[0:3;
        -1:2;
        -2:1;
        -3:0]'*2*sqrt(p21(n)/5);  % Distance matrix
    ...
    for m=1:N  % Number of pages
        for j=1:length(d_4_15)  % Number of columns
            for i=1:length(d_4_15)  % Number of rows
                f15(i)=exp(-d_4_15(i,j)^2/(2*sigma2^2)-z2(m)*d_4_15(i,j)/sigma2^2);
            end
            b15(j)=log(sum(f15))/log(2);  % First sum over i
        end
        x15(m)=sum(b15);  % Second sum over j
    end
    C_2_25(n)=4-0.5*mean(x15);  % Capacity of 16-QAM
end
